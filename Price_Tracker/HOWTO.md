...
Invia /newbot e scegli:

un nome (es. “Price Tracker Bot”),

uno username (deve finire con bot, es. PriceTrackerBot).

BotFather ti restituisce un token tipo:
123456:ABCdefGHIjklMNOpqrsTUVwxyz

Cerca il tuo bot in Telegram e avvialo con /start.

2.2) Ottenere il tuo CHAT_ID
Cerca @RawDataBot su Telegram.

Avvialo con /start.

Ti risponde con un messaggio che contiene il tuo user ID (es. 123456789).

Quel numero è il tuo CHAT_ID.

2.3) Impostare i secrets su GitHub
Vai su GitHub, nella tua repo.
...
Vai su GitHub, nella repo.

Clicca sulla scheda Actions in alto.

Nella colonna di sinistra, clicca su Price tracker.

Clicca su Run workflow (a destra).

Aspetta 2–3 minuti.

4.2) Leggere il log
Clicca sul run appena finito (il primo in lista).

Clicca su check-prices.

Espandi Run price tracker.
...
Cerca il tuo bot (es. @PriceTrackerBot).

Apri la chat e assicurati di avergli mandato almeno una volta /start.
...
Al prossimo giro (o lanciando manualmente il workflow), il nuovo prodotto viene controllato automaticamente.
